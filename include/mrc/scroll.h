...
typedef enum {
  ...
  SCROLL_ERR_SIZE_INVALID,  ///< Requested capacity size is out of acceptable bounds
 ...
 * @param[in]     capacity      The starting capacity will default to 2 if the value provided is < 2
 ...
 * @param[in]       data      The address of the data to be pushed onto the scroll
 * 
 * @return 0 on success, or a non-zero SCROLL_ERR error code on failure
 */
...
/**
 * @brief Pops an element off the end of the scroll, copying the data to a destination buffer
 * 
...
/**
 * @brief Provides a copy of the data at a given index location (non-destructive)
 * 
 * @pre scroll must point to an initialized MRC_Scroll struct
 ...
 * @return 0 on success, or a non-zero SCROLL_ERR error code on failure
 */
MRC_ScrollStatus mrc_scroll_scan(MRC_Scroll *scroll, void *dest);

/**
 * @brief Inserts a new element into the the scroll at a given index, resizing if necessary
 * 
 * @pre scroll must point to an initialized MRC_Scroll struct
 * @pre data must point to a valid memory location of size scroll->unit_size
 ...
 * 
 * @param[in,out]   scroll    The address of the scroll to push data onto
 * @param[in]       data      The address of the data to be inserted into the scroll
...
/**
 * @brief Provides a copy of data at a given location and removes it from the scroll
 * 
 * This is a destructive operation on the scroll
 * 
 * @pre scroll must point to an initialized MRC_Scroll struct
